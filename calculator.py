...
class Calculator:
    ...
    def create_trig_buttons(self):
        ...
        row_index = 1
        col_index = 5  # Новый столбец

        for function, symbol in trig_functions.items():
            button = tk.Button(self.buttons_frame, text=symbol, bg=OFF_WHITE, fg=LABEL_COLOR,
                               font=DEFAULT_FONT_STYLE, borderwidth=0, command=lambda x=function: self.trig_function(x))
            button.grid(row=row_index, column=col_index, sticky=tk.NSEW)
            row_index += 1
    ...
    def create_log_button(self):
        ...
        button.grid(row=4, column=5, sticky=tk.NSEW)  # Новый столбец
    ...
    def trig_function(self, function):
        try:
            if function.startswith("a"):
                self.current_expression = str(math.degrees(math.asin(float(self.current_expression))))
            else:
                self.current_expression = str(getattr(math, function)(math.radians(float(self.current_expression))))
            self.update_label()
        except ValueError:
            self.current_expression = "Error"
            self.update_label()
